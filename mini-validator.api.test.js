...
test("removed APIs are not exposed", () => {
  ...
  assert.equal(v.strictObject, undefined);
  assert.equal(v.parse, undefined);
  assert.equal(v.ValidationError, undefined);
...
test("invalid schemas return schema.invalid issues", () => {
  ...
  assert.equal(v.validate(null, "abc")[0].code, "schema.invalid");
  assert.equal(v.validate({ k: "object" }, {})[0].code, "schema.invalid");
  assert.equal(v.validate({ k: "looseObject" }, {})[0].code, "schema.invalid");
  assert.equal(v.validate({ k: "record" }, { a: 1 })[0].code, "schema.invalid");
});
